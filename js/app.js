...
const giphyTrendingGIFs =
  "https://api.giphy.com/v1/gifs/trending?api_key=" + key;
const giphyTrendingSearchTerms =
  "https://api.giphy.com/v1/trending/searches?api_key=" + key;
const giphySearchSuggestions =
  "https://api.giphy.com/v1/gifs/search/tags?api_key=" + key + "&q=";
const giphySearchGIFs =
  "https://api.giphy.com/v1/gifs/search?api_key=" + key + "&q=";

// Memoria temporal de la búsqueda
let resultadosMemoria;
// Memoria temporal de los trending
let resultadosMemoriaTrending;
...
//Menú Mobile
const menuHamburguesa = document.getElementById("nav-ham-div");
const aMenuHamburguesa = document.getElementById("nav-ham");
...
const searchBarLupaPlaceholder = document.getElementById(
...
const misGIFOSPaginationList = document.getElementById(
  "section-mis-gal-pag-list"
);
// Crear GIFOS
const crearGIFOS = document.getElementById("section-cre");
const crearGIFOSrollo1 = document.getElementById("rollo-1");
const crearGIFOSrollo2 = document.getElementById("rollo-2");
...
const crearGIFOSVideoRecorded = document.getElementById(
  "section-cre-video-recorded"
);
...
const crearGIFOSVideoOverlay = document.getElementById(
  "section-cre-video-recorded-overlay"
);
const crearGIFOSVideoOverlayActions = document.getElementById(
  "section-cre-video-recorded-overlay-actions"
);
const crearGIFOSVideoOverlayIcon = document.getElementById(
  "section-cre-video-recorded-overlay-icon"
);
const crearGIFOSVideoOverlayText = document.getElementById(
...
async function fetchGif(json, divToPlace, index) {
  ...
  divGif.style.background =
    "url(" +
    gifURL +
    ") no-repeat center / cover, url('img/loading.gif') no-repeat center";
  ...
  const unfavoriteGifCallback = () => {
    ...
    fetchedGifOptionFavorite.removeEventListener(
      "click",
  ...
  const expandGifCallback = () => {
    expandGif(index, gifURL, usuario, titulo, json);
  };
    ...
    .then((json) => {
      ...
      for (i = 0; i < json.data.length; i++) {
        ...
        suggestionItem.innerHTML =
          '<div class="search-bar-lupa-search"></div>' +
          searchSuggestionFetched;
...
const expandGif = (index, gifURL, usuario, titulo, json) => {
  ...
  const slideLeft = () => {
    prevIndex = index - 1;
    prevURL = json.data[prevIndex].images.downsized.url;
    prevUser = json.data[prevIndex].username;
    prevTitle = json.data[prevIndex].title;
    overlayDivSlideLeft.removeEventListener("click", slideRight);
    overlayDiv.remove();
    expandGif(prevIndex, prevURL, prevUser, prevTitle, json);
  };

  const slideRight = () => {
    nextIndex = index + 1;
    nextURL = json.data[nextIndex].images.downsized.url;
    nextUser = json.data[nextIndex].username;
    nextTitle = json.data[nextIndex].title;
    overlayDivSlideRight.removeEventListener("click", slideRight);
    overlayDiv.remove();
    expandGif(nextIndex, nextURL, nextUser, nextTitle, json);
  };
  ...
  } else if (index < json.data.length && index > 0) {
    overlayDivColRight.appendChild(overlayDivSlideRight);
    overlayDivSlideRight.addEventListener("click", slideRight);
    overlayDivColLeft.appendChild(overlayDivSlideLeft);
    overlayDivSlideLeft.addEventListener("click", slideLeft);
...
const listarResultados = (
  ...
  console.warn(
    "Voy a ponerle activo a: " +
      searchPaginationList.children[paginaIndex].innerHTML
  );
    ...
    .then((json) => {
      ...
      if (cantidadResultados > 0) {
        ...
        const mostrarMas = () => {
          let indexDesde = paginaIndex * resultadosAMostrar;
          let indexHasta =
            paginaIndex * resultadosAMostrar + resultadosAMostrar;
        ...
        listarResultados(
          paginaIndex * resultadosAMostrar,
          paginaIndex * resultadosAMostrar + resultadosAMostrar,
          paginaIndex,
          paginaActual,
          cantidadPaginas,
...
const fetchStream = () => {
  ...
  crearGIFOSDescription.innerHTML =
    "El acceso a tu cámara será válido sólo<br>por el tiempo en el que estés creando el GIFO.";
...
const recordStream = () => {
  crearGIFOSrollo1.classList.add("rotating");
  crearGIFOSrollo2.classList.add("rotating");
...
const uploadStream = () => {
  crearGIFOSrollo1.classList.remove("rotating");
  crearGIFOSrollo2.classList.remove("rotating");
...
//Links menú en mobile
aMenuHamburguesa.addEventListener("click", (e) => {
  menuHamburguesa.classList.toggle("hide");
});

aMenuCloseMobile.addEventListener("click", (e) => {
  menuHamburguesa.classList.toggle("hide");
});

aMisFavoritosMobile.addEventListener("click", (e) => {
  menuHamburguesa.classList.toggle("hide");
  e.preventDefault;
  e.stopPropagation;
  changeSection("favoritos");
...
aMisGIFOSMobile.addEventListener("click", (e) => {
  menuHamburguesa.classList.toggle("hide");
  e.preventDefault;
  e.stopPropagation;
...
aModoNocturnoMobile.addEventListener("click", (e) => {
  ...
  if (modoNocturno === false) {
    ...
    aModoNocturnoMobile.innerHTML = "<li>Modo Diurno</li>";
    modoNocturno = true;
  } else {
    body.classList.remove("dark-mode");
    aModoNocturnoMobile.innerHTML = "<li>Modo Nocturno</li>";
    modoNocturno = false;
  }
});

//Links menú en Desktop
